  ...
  return (
    <div className="trip-container">
      ...
      <table className="trip-table">
        <thead>
          <tr>
            <th onClick={() => requestSort("startStop")}>
              Dept. {getSortIcon("startStop")}
            </th>
            <th onClick={() => requestSort("started")}>
              Dep. Time {getSortIcon("started")}
            </th>
            <th onClick={() => requestSort("endStop")}>
              Dest. {getSortIcon("endStop")}
            </th>
            <th onClick={() => requestSort("finished")}>
              Arr. Time {getSortIcon("finished")}
            </th>
            <th onClick={() => requestSort("busId")}>
              Bus {getSortIcon("busId")}
            </th>
            <th onClick={() => requestSort("companyId")}>
              Company {getSortIcon("companyId")}
            </th>
            <th onClick={() => requestSort("chargedAmount")}>
              Amount {getSortIcon("chargedAmount")}
            </th>
            <th onClick={() => requestSort("pan")}>
              Account {getSortIcon("pan")}
            </th>
        ...
        <tbody>
          ...
          {trips.length > 0
            ? trips.map((trip, index) => (
                <tr key={index}>
                  <td data-label="Dept." title={trip.startStop}>
                    {trip.startStop}
                  </td>
                  <td data-label="Dep. Time">{formatDate(trip.started)}</td>
                  <td data-label="Dest." title={trip.endStop}>
                    {trip.endStop}
                  </td>
                  ...
                  <td data-label="Account" className="account-cell">
                    {trip.pan}
                  </td>
